// Project Euler, problem 11: <Title> (http://projecteuler.net/problem=11). 
...
class P11 {
	...
	for(int x = 0; x < width; x++){
		...
		for(int dir=1 ; dir <= 8; dir++){
		    if(validRange(x,y, dir)){
			int product = getProduct(matrix, x,y, dir);
    ...
    private static boolean validRange(int x,int y, int direction ){
	switch(direction){
	case N: 
	    return (y - range >= 0);	     
	case S: 
	    return (y + range < width);	     
	case E:
	    return(x+ range < width);	     
	case W:
	    return(x - range >= 0 );	     
	case NW:
	    return (y - range >= 0 && x  - range >= 0);	     
	case NE:
	    return (y - range >= 0 && x  + range < width);	     
	case SW:
	    return (y + range < width && x  - range >= 0);	     
	case SE:
	    return (y + range < width && x  + range < width);	     	   
	default: 
	    return false; 
	}		    
    }

    private static int getProduct(int[][] matrix, int x, int y, int direction){
	int product = 1; 
